...
- **What it takes from the reports that already exist.** **Notes signed < 24h**, **Median time from session end to signed note** and **Golden Thread completeness** are all on the Provider journey. What “advanced” adds to those is the question this report has to answer first.
...
- **Session-end timestamp** — the whole documentation-timeliness family depends on it. Confirm it exists in the model before anything here is specified.
- **Overlap with the Provider journey** — three documentation measures already live there.
