...
- **What it takes from the reports that already exist.** **Sessions held — billed vs unbilled** sits on the Provider journey today. Moving it, repeating it or leaving it there is a decision, not a detail.
...
- **Overlap with the Provider journey** — the one session measure that exists is there. Decide whether this report takes it over.
