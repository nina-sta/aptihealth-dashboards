...
- **What it takes from the reports that already exist.** The Provider journey measures the group. A scorecard measures a named person, which is a different report and a different conversation — and one this repo has so far kept internal on purpose.
...
- **What a provider is scored on** — not agreed.
- **Who sees the score, and what follows from it** — not agreed. A scorecard nobody acts on is decoration; one that is acted on needs the rules written down first.
